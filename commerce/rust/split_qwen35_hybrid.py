...
def run_hybrid_split():
    ...
    for name, tensor in tqdm(full_sd.items(), desc="Hybrid Quantizing"):
        # 1. Vision weights: ALWAYS BF16 (No quantization)
        if "model.visual." in name:
            vision_tensors[name] = tensor.to(torch.bfloat16)
            continue
            
        # 2. Other weights: Target for Q2
        ...
        # Only quantize heavy Linear weights
        ...
        if is_linear_weight and tensor.numel() > 1024:
            ...
            if scales is not None:
                # Keep original name for packed data to avoid "tensor not found"
    ...
    # Save split files
    print("Saving Hybrid Split files...")
    ...
    print("Hybrid Generation Complete: Vision (BF16), Language (Q2). Original names preserved.")
