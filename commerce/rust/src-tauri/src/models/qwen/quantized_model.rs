...
impl QuantizedQwenVLTextAttention {
    ...
    pub fn forward(
        ...
        for block in &self.kv_blocks {
            ...
            // 🌟 [CRITICAL FIX] 이미 k_block과 v_block은 위쪽 로직(VRAM/RAM/SSD 로드)에서 정확한 데이터 길이(inner.len)만큼 
            // narrow() 되어 올라오므로, 여기서는 그대로 받아 쓰기만 하면 됩니다. 스코프 밖의 b_len을 참조할 필요가 없습니다.
            let k = k_block; 
            let v = v_block; 
...
impl QuantizedQwenVLTextModel {
    ...
    async fn process_single_layer(
        ...
        // 🌟 속도 최적화를 위해 보폭을 chunk_size와 일치하는 2048로 롤백
        ...
        for (l_idx, layer) in self.layers.iter_mut().enumerate() {
            for block in &mut layer.self_attn.kv_blocks {
                ...
                if inner.k_cache.is_some() && is_dirty {
                    let k_raw = inner.k_cache.as_ref().unwrap();
                    let v_raw = inner.v_cache.as_ref().unwrap();
                    let b_len = inner.len;

                    // 🌟 [CRITICAL FIX] 디스크에 구울 때도 쓰레기값이 딸려가서 다음 로딩 때 뇌를 오염시키지 않도록 진짜 길이만큼만 잘라냅니다.
                    let k_sliced = if k_raw.dim(2).unwrap_or(0) > b_len { k_raw.narrow(2, 0, b_len).unwrap_or_else(|_| k_raw.clone()) } else { k_raw.clone() };
                    let v_sliced = if v_raw.dim(2).unwrap_or(0) > b_len { v_raw.narrow(2, 0, b_len).unwrap_or_else(|_| v_raw.clone()) } else { v_raw.clone() };

                    let k_shape_u32: Vec<u32> = k_sliced.shape().dims().iter().map(|&x| x as u32).collect();
                    
                    // [CRITICAL FIX] VRAM -> RAM 오프로딩 시 PCIe 대역폭 폭발을 막기 위해, 
                    // CPU로 넘기기 전 VRAM 내부에서 미리 contiguous()로 텐서를 하나로 묶은 뒤 단번에 쏘아보냅니다!
                    let target_dtype = if k_sliced.device().is_cuda() { candle_core::DType::BF16 } else { candle_core::DType::F32 };
                    block_groups.entry((inner.offset, inner.index)).or_default().push(LayerKVDump {
                        ...
                        k_shape: Tensor::from_vec(k_shape_u32, (k_sliced.shape().dims().len(),), &candle_core::Device::Cpu).unwrap(),
                        raw_k: Some(k_sliced.contiguous().unwrap_or_else(|_| k_sliced.clone()).to_dtype(target_dtype).unwrap_or_else(|_| k_sliced.clone()).to_device(&candle_core::Device::Cpu).unwrap_or_else(|_| k_sliced.clone())), 
                        raw_v: Some(v_sliced.contiguous().unwrap_or_else(|_| v_sliced.clone()).to_dtype(target_dtype).unwrap_or_else(|_| v_sliced.clone()).to_device(&candle_core::Device::Cpu).unwrap_or_else(|_| v_sliced.clone())),
                    });
