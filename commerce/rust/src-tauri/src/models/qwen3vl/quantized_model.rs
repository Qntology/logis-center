...
impl QuantizedQwen3VLTextDecoderLayer {
    ...
    pub fn get_kv_len(&self) -> usize { self.self_attn.get_kv_len() }
    pub fn drop_kv_storage(&mut self) -> Result<()> { self.self_attn.drop_kv_storage() }
    pub fn device(&self) -> &Device { self.input_layernorm.weight().device() }
    pub fn save_kv_cache(&mut self, path: &Path, clear: bool, block_size: usize) -> Result<()> { self.self_attn.save_kv_cache(path, clear, block_size) }
    pub fn offload_kv_cache(&mut self, path: &Path, block_size: usize) -> Result<()> { self.self_attn.offload_kv_cache(path, block_size) }
...
impl QuantizedQwen3VLTextModel {
    pub fn new_with_mmap(
        ...
        let (embed_tokens, actual_hidden_size) = if let Ok(tensor) = ct.tensor(&mut reader, &token_emb_name, device) {
             ...
             let h = tensor.dim(1)?;
             (Embedding::new(tensor, h), h)
        } else { return Err(anyhow!("Failed to load embedding.")); };
        ...
        let layer_prefix = if ct.tensor_infos.keys().any(|k| k.starts_with("blk.0.")) { "blk.0." } else { "model.layers.0." };
        for (name, info) in &ct.tensor_infos {
            if name.starts_with(layer_prefix) {
                let elements: usize = info.shape.elem_count();
                layer_weight_size += ((elements / info.ggml_dtype.block_size()) * info.ggml_dtype.type_size()) as u64;
            }
        }
        ...
        for _ in 0..config.num_hidden_layers {
            if current_device.is_cuda() && is_vram_checked {
                 if simulated_free_vram > ( (cost_per_layer as f64 * 1.2) as u64 + kv_reserve + 200_000_000 ) {
                     simulated_free_vram = simulated_free_vram.saturating_sub(cost_per_layer);
                     layer_devices.push(current_device.clone());
                 } else { layer_devices.push(Device::Cpu); }
            } else { layer_devices.push(current_device.clone()); }
        }
        ...
        let layers: Result<Vec<_>> = pool.install(|| {
            (0..final_config.num_hidden_layers).into_par_iter().zip(layer_devices).map(|(layer_idx, layer_device)| {
                ...
                QuantizedQwen3VLTextDecoderLayer::new(final_config, ct, &mut local_cursor, &prefix, &layer_device, layer_dtype, layer_idx, baking_only)
            }).collect()
        });
        ...
        Ok(Self { embed_tokens, layers, norm, rotary_emb, mrope_section, mmap: mmap_handle, baking_only, is_forced_cpu, is_disk_swap, active_session_id: None, pinned_layer_count: 0, current_kv_len: 0 })
    }
    ...
    pub fn new<R: std::io::Seek + std::io::Read>(
        ...
        for layer_idx in 0..config.num_hidden_layers {
            ...
            if layer_device.is_cuda() && is_vram_checked {
                 ...
                 if simulated_free_vram > ( (actual_cost as f64 * 1.05) as u64 + kv_reserve + 50_000_000 ) {
            ...
            let gguf_blk = format!("blk.{layer_idx}");
            let prefix = if ct.tensor_infos.contains_key(&format!("{}.attn_norm.weight", gguf_blk)) { gguf_blk } else { standard };
            layers.push(QuantizedQwen3VLTextDecoderLayer::new(config, ct, reader, &prefix, &layer_device, layer_dtype, layer_idx, baking_only)?);
        }
        ...
        Ok(Self { embed_tokens, layers, norm, rotary_emb, mrope_section, mmap: None, baking_only, is_forced_cpu, is_disk_swap, active_session_id: None, pinned_layer_count: 0, current_kv_len: 0 })
    }
    ...
    pub fn forward(
        ...
        let position_ids = match position_ids {
            Some(ids) => ids.clone(),
            None => Tensor::arange(seqlen_offset as u32, (seq_len + seqlen_offset) as u32, &target_device)?.unsqueeze(0)?.unsqueeze(0)?.broadcast_as((3, b_size, seq_len))?,
        };
        let (cos, sin) = self.rotary_emb.forward(&position_ids, target_dtype, self.mrope_section.clone())?;
        let attention_mask = if seq_len <= 1 { None } else {
            let mask = prepare_causal_attention_mask(b_size, seq_len, seqlen_offset, &target_device)?;
        ...
        for (layer_idx, layer) in self.layers.iter_mut().enumerate() {
            ...
            if self.is_disk_swap && !is_pinned {
                if let Some(session_id) = &self.active_session_id {
                    ...
                    if kv_path.exists() { let _ = layer.load_kv_cache(&kv_path, &target_device, 0, 0); }
                }
            }
            if is_on_cpu { layer.to_device(&target_device)?; }
            xs = layer.forward(&xs, &cos, &sin, attention_mask.as_ref())?;
            if let Some(deepstack_embeds) = deepstack_visual_embeds.as_ref() {
                if layer_idx < deepstack_embeds.len() {
                    let mask = visual_pos_masks.unwrap().to_device(&target_device)?;
                    let embed = deepstack_embeds[layer_idx].to_device(&target_device)?.to_dtype(xs.dtype())?;
    ...
    pub fn clear_kv_cache(&mut self) { for layer in self.layers.iter_mut() { layer.clear_kv_cache() } self.current_kv_len = 0; }
    pub fn get_kv_len(&self) -> usize { self.current_kv_len }
    pub fn drop_kv_storage(&mut self) -> Result<()> { for layer in self.layers.iter_mut() { layer.drop_kv_storage()?; } Ok(()) }
    ...
    pub fn inject_live_kv_bitkv(&mut self, k_anchors: &[Tensor], k_packed: &[Tensor], k_scales: &[Tensor], v_anchors: &[Tensor], v_packed: &[Tensor], v_scales: &[Tensor], original_shape: &[usize]) -> Result<()> {
        ...
        for (i, layer) in self.layers.iter_mut().enumerate() {
            if i < k_anchors.len() {
                let k = layer.self_attn.decompress_from_bitkv(&k_anchors[i].to_device(&target_device)?, &k_packed[i].to_device(&target_device)?, &k_scales[i].to_device(&target_device)?, original_shape)?;
                let v = layer.self_attn.decompress_from_bitkv(&v_anchors[i].to_device(&target_device)?, &v_packed[i].to_device(&target_device)?, &v_scales[i].to_device(&target_device)?, original_shape)?;
                layer.self_attn.inject_live_kv_direct(&k.to_dtype(target_dtype)?, &v.to_dtype(target_dtype)?)?;
            }
        }
        Ok(())
    }
    ...
    pub fn to_device(&mut self, device: &Device) -> Result<()> {
        ...
        for layer in self.layers.iter_mut() { layer.to_device(device)?; }
        self.norm.to_device(device)?; Ok(())
    }
    pub fn rebalance_layers(&mut self, device_id: usize, _context_len: usize) -> Result<()> {
        if self.is_forced_cpu { return Ok(()); }
        let nvml = nvml_wrapper::Nvml::init().ok();
        let mut free_vram = 0;
        if let Some(nvml_inst) = &nvml { if let Ok(dev) = nvml_inst.device_by_index(device_id as u32) { if let Ok(mem) = dev.memory_info() { free_vram = mem.free; } } }
        if free_vram > 0 && free_vram < 300_000_000 {
            for layer in self.layers.iter_mut().rev() { if layer.device().is_cuda() { layer.to_device(&Device::Cpu)?; break; } }
        } else if free_vram > 800_000_000 {
            let target_device = Device::new_cuda(device_id)?;
            for layer in self.layers.iter_mut() { if layer.device().is_cpu() { layer.to_device(&target_device)?; break; } }
        }
        Ok(())
    }
}
...
impl QuantizedQwen3VLModel {
    pub fn new_with_mmap(
        ...
        let language_model = QuantizedQwen3VLTextModel::new_with_mmap(t_config, ct_main, main_mmap_handle.clone(), "model", text_device, text_device_id, dtype, kv_reserve, baking_only, false)?;
        ...
        let lm_head = if let Ok(l) = get_qlinear(ct_main, &mut reader_main, "lm_head", text_device, head_dtype) { l }
                      else if let Ok(l) = get_qlinear(ct_main, &mut reader_main, "output", text_device, head_dtype) { l }
                      else { get_qlinear(ct_main, &mut reader_main, "token_embd", text_device, head_dtype)? };
    ...
    pub fn new<R: std::io::Seek + std::io::Read, R2: std::io::Seek + std::io::Read>(
        ...
        let visual = Qwen3VLVisionModel::new(v_config.clone(), vb_visual.pp("visual"))?;
        let t_config = config.text_config.as_ref().ok_or(anyhow!("Missing text_config"))?;
        let language_model = QuantizedQwen3VLTextModel::new(t_config, ct_main, reader_main, "model", text_device, text_device_id, dtype, kv_reserve, baking_only, false)?;
        let head_dtype = if text_device.is_cpu() { DType::F32 } else { dtype };
        ...
        } else { QLinear::new(QMatMul::Tensor(Tensor::zeros((1, 1), head_dtype, text_device)?), None, text_device.clone()) };
...
impl QuantizedQwen3TextModel {
    pub fn new_with_mmap(config: &Qwen3VLConfig, ct_main: &gguf_file::Content, mmap_handle: Option<Arc<Mmap>>, text_device: &Device, text_device_id: usize, dtype: DType, kv_reserve: u64, baking_only: bool, single_layer_mode: bool, is_disk_swap: bool) -> Result<Self> {
        ...
        let language_model = QuantizedQwen3VLTextModel::new_with_mmap(&t_config, ct_main, mmap_handle.clone(), "model", text_device, text_device_id, dtype, kv_reserve, baking_only, true)?;
        let lm_head = if !baking_only {
            let mmap = mmap_handle.as_ref().map(|m| &m[..]).unwrap_or(&[]);
            let mut reader = std::io::Cursor::new(mmap);
            let head_dtype = if text_device.is_cpu() { DType::F32 } else { dtype };
    ...
    pub fn new<R: std::io::Seek + std::io::Read>(config: &Qwen3VLConfig, ct_main: &gguf_file::Content, reader_main: &mut R, text_device: &Device, text_device_id: usize, dtype: DType, kv_reserve: u64, baking_only: bool, single_layer_mode: bool, is_disk_swap: bool) -> Result<Self> {
        println!("[MODEL] Loading as Pure Text (Baking-Only: {}, Single-Layer: {}, DiskSwap: {})", baking_only, single_layer_mode, is_disk_swap);
        let mut t_config = config.text_config.as_ref().ok_or(anyhow!("Missing text_config"))?.clone();
        if single_layer_mode { t_config.num_hidden_layers = 1; }
        let language_model = QuantizedQwen3VLTextModel::new(&t_config, ct_main, reader_main, "model", text_device, text_device_id, dtype, kv_reserve, baking_only, true)?;
        let lm_head = if !baking_only {
            let head_dtype = if text_device.is_cpu() { DType::F32 } else { dtype };
