...
impl ModelVariant {
    ...
    pub fn set_baking(&mut self, b: bool) {
        if let Self::Standard(m) = self { m.set_baking(b); }
    }
...
impl Qwen3VLGenerateModel {
    ...
    pub fn prefill_text_only(&mut self, text: &str, cancel_token: Option<Arc<AtomicBool>>, _relay_target: Option<&mut Qwen3VLGenerateModel>, auto_save_path: Option<&std::path::Path>) -> Result<()> {
        ...
        let total_tokens = token_ids.len(); let chunk_size = 256;
    ...
    pub fn prefill_only(&mut self, mes: ChatCompletionParameters, cancel_flag: Option<Arc<AtomicBool>>, session_id: Option<String>, _relay_target: Option<&mut Qwen3VLGenerateModel>) -> Result<usize> {
        ...
        let total_tokens = full_input_ids_vec.len(); let mut current_pos = self.get_kv_len(); 
        let chunk_size = 256;
    ...
    pub fn generate(&mut self, mes: ChatCompletionParameters, cancel_flag: Option<Arc<AtomicBool>>, session_id: Option<String>) -> Result<String> {
        ...
        while local_pos < total_tokens {
            ...
            if local_pos + c_size >= total_tokens && generated_text.is_empty() { c_size = (total_tokens - local_pos).saturating_sub(1); }
            if c_size == 0 { break; }
        ...
        for _i in 0..mes.max_tokens.unwrap_or(2048) {
            ...
            seqlen_offset += seq_len; local_pos = total_tokens; pixel_values = None;
        }
        Ok(generated_text)
    }

    pub fn get_kv_len(&self) -> usize { match &self.qwen3_vl { ModelVariant::QuantizedVL(m) => m.language_model.get_kv_len(), ModelVariant::QuantizedText(m) => m.language_model.get_kv_len(), ModelVariant::Standard(m) => m.language_model.get_kv_len() } } 
    ...
    pub fn to_device(&mut self, d: &Device) -> Result<()> { match &mut self.qwen3_vl { ModelVariant::QuantizedVL(m) => m.to_device(d)?, ModelVariant::QuantizedText(m) => m.to_device(d)?, ModelVariant::Standard(m) => m.to_device(d)? }; self.text_device = d.clone(); self.vision_device = d.clone(); Ok(()) }
    pub fn clear_kv_cache(&mut self) { match &mut self.qwen3_vl { ModelVariant::QuantizedVL(m) => m.clear_kv_cache(), ModelVariant::QuantizedText(m) => m.clear_kv_cache(), ModelVariant::Standard(m) => m.language_model.clear_kv_cache() } }
}