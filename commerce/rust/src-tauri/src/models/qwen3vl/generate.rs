...
impl Qwen3VLGenerateModel {
    ...
    /// [SSD-BRIDGE] Wrapper for simple full-dump saving
    pub fn save_kv_to_disk(&mut self, path: &std::path::Path) -> anyhow::Result<()> {
        let block_size = 1024;
        match &mut self.qwen3_vl {
            ModelVariant::QuantizedVL(m) => m.save_kv_cache(path, false, block_size),
            ModelVariant::QuantizedText(m) => m.save_kv_cache(path, false, block_size),
            ModelVariant::Standard(_) => Err(anyhow::anyhow!("SSD Bridge not supported for Standard models")),
        }
    }
