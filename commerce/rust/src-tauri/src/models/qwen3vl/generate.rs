                        ...
                        if let Err(panic_info) = result {
                            ...
                            let _ = SLOT_MANAGER.request_tx.blocking_send(SlotRequest::Release { 
                                ...
                                SlotTask::Load(load) => {
                                    ...
                                    tokio::spawn(async move {
                                        let res = async {
                                            ...
                                            let (ka_data, o_s, a_cnt) = if let Ok(t_view) = st.tensor("k_anchors") {
                                                    ...
                                                    if let Ok(mut reg) = registry.entries.write() {
                                                        if b_idx < reg.len() { 
                                                            ...
                                                            if is_relay {
                                                                ...
                                                                println!("[WORKER-LOAD] << [SUCCESS] RELAY Block {} populated for ALL layers (Slot {}).", b_idx, s_id);
                                                            } else {
                                                                reg[b_idx].location[l_idx] = crate::models::qwen3vl::quantized_model::KVLocation::RAM; 
                                                                let mut cache = reg[b_idx].bitkv_cache.write().unwrap();
                                                                cache[l_idx] = Some(metadata);
                                                                println!("[WORKER-LOAD] << [SUCCESS] Layer {} Block {} loaded (Slot {}).", l_idx, b_idx, s_id);
                                                            }
                                                        }
                                                    }
                                                }
                                ...
                                SlotTask::ChunkedLoad(chunk) => {
                                    ...
                                    println!("[WORKER-CHUNK] >> [START] CHUNKED LOAD for {} blocks (Sequential Mode).", num_blocks);
                                    
                                    // [FIX] Sequential Processing Loop (Ticket System)
                                    // Instead of spawning 40 concurrent tasks, we process them one by one (or in small batches)
                                    // to prevent IO/CPU choking and ensure orderly completion.
                                    let mut futures = futures::stream::iter(0..num_blocks)
                                        .map(|i| {
                                            let l_idx = chunk.layer_indices[i];
                                            let s_id = chunk.slot_ids[i];
                                            let block: crate::models::qwen3vl::quantized_model::KVBlock = chunk.shared_blocks[i].clone();
                                            ...
                                            async move {
                                                let (b_idx, target_path) = {
                                                    match block.inner.read() {
                                                        Ok(inner) => {
                                                            let path = inner.ssd_path.clone().or_else(|| {
                                                                if let Ok(reg) = reg_inner.entries.read() {
                                                                    if inner.index < reg.len() { reg[inner.index].ssd_path.clone() } else { None }
                                                                } else { None }
                                                            });
                                                            (inner.index, path)
                                                        ...
                                                        if let (Some(ka), Some(kp), Some(ks), Some(va), Some(vp), Some(vs)) = (ka_data, st.tensor("k_packed").ok(), ex_v("k_scales"), ex_v("v_anchors"), st.tensor("v_packed").ok(), ex_v("v_scales")) {
                                                            ...
                                                            if let Ok(mut inner) = block.inner.write() {
                                                                ...
                                                                if let Ok(mut reg) = reg_inner.entries.write() {
                                                                    if b_idx < reg.len() { 
                                                                        let is_relay = target_path.file_name().map(|n: &std::ffi::OsStr| n.to_string_lossy().contains("layer_relay_kv")).unwrap_or(false);
                                                                        if is_relay {
                                                                            for i in 0..28 { reg[b_idx].location[i] = crate::models::qwen3vl::quantized_model::KVLocation::RAM; }
                                                                            let mut cache = reg[b_idx].bitkv_cache.write().unwrap();
                                                                            for i in 0..28 { cache[i] = Some(metadata.clone()); }
                                                ...
                                                // Release Slot *after* work is done
                                                SLOT_MANAGER.release_slot(s_id).await;
                                            }
                                        })
                                    ...
                                    println!("[WORKER-CHUNK] << [FINISH] All {} blocks processed.", num_blocks);
                                }
