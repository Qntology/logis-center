...
// [2026-SPEED-MASTER] Ultra-Fast Tiled Bit-serial Matmul
// Restores the previous tiled performance while enforcing strict boundary safety.
__global__ void bit_serial_matmul_kernel_shuffled(
    ...
    // 32 tokens x 8 channels per block
    const int TILE_M = 32;
    ...
    for (int kb = 0; kb < k_blocks; ++kb) {
        // Collaborative load: ONE thread per row (n_sub == 0) loads for all 8 output channels
        if (n_sub == 0) {
            uint32_t bts = 0;
            if (m < M) {
                #pragma unroll
                for (int b = 0; b < 32; ++b) {
                    int k_idx = kb * 32 + b;
                    if (k_idx < K) {
                        if (input[m * K + k_idx] >= 0.0f) bts |= (1u << b);
                    }
                }
            }
...
// [FAST-ATTENTION] High-speed online softmax attention
