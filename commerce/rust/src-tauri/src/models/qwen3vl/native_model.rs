    ...
    pub fn grow(&mut self, needed_len: usize, n_kv: usize, head_dim: usize, device_id: i32) {
        if needed_len > self.capacity {
            ...
            unsafe {
                ...
                let mut res_k = 1; let mut res_v = 1;
                if device_id >= 0 && device_id < 8 {
                    res_k = cl.cuMemAlloc_v2(&mut new_kp, k_bytes) as i32;
                    res_v = cl.cuMemAlloc_v2(&mut new_vp, v_bytes) as i32;
                }
                
                if res_k != 0 || res_v != 0 {
                    if device_id < 8 { return; }
                    else { self.capacity = new_cap; return; }
                }
                ...
                self.capacity = new_cap;
                if device_id < 8 { cl.cuStreamSynchronize(std::ptr::null_mut()); }
            }
        }
    }
    pub fn clear(&mut self) { self.current_len = 0; }
...
fn log_tensor_health(name: &str, data: &[f16], shape: &[usize]) {
    ...
    let mut min = f32::MAX; let mut max = f32::MIN; let mut sum_abs = 0.0f32;
    for &v in data {
        let f = v.to_f32();
    ...
    println!("[HEALTH] {:<40} | Shape: {:<12?} | Min: {:>8.4} | Max: {:>8.4} | AbsAvg: {:>8.4}", name, shape, min, max, avg_abs);
}
    ...
    for no in 0..(n_padded / 8) {
        for skb in 0..src_k_blocks {
            for sub_n in 0..8 {
                ...
                for b in 0..32 {
                    let k_idx_base = skb * 32 + b; if k_idx_base >= src_k { break; }
                    let mut bit_sum = 0.0f32;
                    for sl in 0..slice_count {
                        let s_idx = sl * slice_stride + (no * src_k_blocks + skb) * 8 + sub_n;
                        if ((packed[s_idx] >> b) & 1) == 1 { bit_sum += (1 << sl) as f32; }
                    }
                    let final_val = if slice_count == 4 { (bit_sum - 8.0) * scales[n_idx].to_f32() }
                                    else { (if bit_sum >= 1.0 { 1.0 } else { -1.0 }) * scales[(no * src_k_blocks + skb) * 8 + sub_n].to_f32() };
                    for r in 0..ratio {
                        let tkidx = k_idx_base + (r * src_k);
                        if tkidx < target_k { out[n_idx * target_k + tkidx] = f16::from_f32(final_val); }
                    }
                }
            }
        }
...
impl NativeLinear {
            ...
            LinearVariant::Standard { weight, bias } => {
                ...
                if self.device_id >= 0 {
                    if let Some(w_gpu) = weight.gpu_ptr {
                        let mut di = 0; let mut dog = 0;
                        if let Some((si, so, _)) = global_scratch {
                            di = si.lock().unwrap().as_ref().map(|(p, _)| p.0 as CUdeviceptr).unwrap_or(0);
                            dog = so.lock().unwrap().as_ref().map(|(p, _)| p.0 as CUdeviceptr).unwrap_or(0);
                        }
                        if di != 0 && dog != 0 {
                            unsafe {
                                let cl = crate::models::qwen3vl::native_backend::lib();
    ...
    pub fn move_to_gpu(&mut self, device_id: i32) -> anyhow::Result<()> {
        ...
        match &mut self.variant {
            ...
            LinearVariant::BitSliced4 { weight_packed, scales, bias } => { weight_packed.move_to_gpu(device_id)?; scales.move_to_gpu(device_id)?; if let Some(b) = bias { b.move_to_gpu(device_id)?; } },
        }
        Ok(())
    }
}
...
impl NativeLayer {
    pub fn move_to_gpu(&mut self, device_id: i32) -> anyhow::Result<()> {
        self.device_id = device_id;
        self.input_layernorm.move_to_gpu(device_id)?; self.post_attention_layernorm.move_to_gpu(device_id)?;
        if let Some(ref mut qn) = self.q_norm { qn.move_to_gpu(device_id)?; } if let Some(ref mut kn) = self.k_norm { kn.move_to_gpu(device_id)?; }
        self.q_proj.move_to_gpu(device_id)?; self.k_proj.move_to_gpu(device_id)?; self.v_proj.move_to_gpu(device_id)?; self.o_proj.move_to_gpu(device_id)?;
        self.gate_proj.move_to_gpu(device_id)?; self.up_proj.move_to_gpu(device_id)?; self.down_proj.move_to_gpu(device_id)?;
        Ok(())
    }
    ...
    pub fn forward<'a>(
        ...
        if self.device_id >= 0 && self.device_id < 8 && !self.gpu_broken.load(Ordering::Relaxed) {
            if let Some((si, so, sr)) = global_scratch {
                let req_bytes = q_len * h_s.max(self.gate_proj.out_features) * 4;
                let di = si.lock().unwrap().as_ref().map(|(p, s)| if *s >= req_bytes { p.0 as CUdeviceptr } else { 0 }).unwrap_or(0);
                let dog = so.lock().unwrap().as_ref().map(|(p, s)| if *s >= req_bytes { p.0 as CUdeviceptr } else { 0 }).unwrap_or(0);
                let drg = sr.lock().unwrap().as_ref().map(|(p, s)| if *s >= req_bytes { p.0 as CUdeviceptr } else { 0 }).unwrap_or(0);
                ...
                if di != 0 && dog != 0 && drg != 0 {
                    unsafe {
                        ...
                        {
                            let mut rg = rope_cache_gpu.lock().unwrap();
                            if rg.is_none() {
                                ...
                                let _ = ct.move_to_gpu(self.device_id); let _ = st.move_to_gpu(self.device_id); *rg = Some((ct, st));
                            }
                            if let Some((ref c, ref s)) = *rg {
                                crate::models::qwen3vl::native_backend::native_cuda_apply_rope(dq, dk, c.gpu_ptr.unwrap().0 as CUdeviceptr, s.gpu_ptr.unwrap().0 as CUdeviceptr, q_len, s_o, n_h, n_kv, h_d);
                            }
                        }
                        ...
                        crate::models::qwen3vl::native_backend::native_bit_serial_attn_gpu_buffered(&[], gkg.k_ptr.unwrap(), gkg.v_ptr.unwrap(), n_h, n_kv, h_d, n_tok, self.device_id as usize, dq, dog, f_alpha, self.q_proj.src_in / n_h, true);
                        ...
                        self.down_proj.forward_gpu(di, dog, q_len);
                        if self.down_proj.out_features > self.down_proj.src_out { crate::models::qwen3vl::native_backend::native_cuda_hybrid_repeat(dog, self.down_proj.src_out, self.down_proj.out_features, q_len); }
                        crate::models::qwen3vl::native_backend::native_cuda_add_inplace(dog, drg, x.len());
                        let rs = std::slice::from_raw_parts_mut(out_ptr, x.len()); cl.cuMemcpyDtoH_v2(rs.as_mut_ptr() as *mut _, dog, x.len() * 2); return rs;
                    }
                }
            }
        }
        ...
        self.q_proj.forward_into(&cn, &mut workspace.q, global_scratch); self.k_proj.forward_into(&cn, &mut workspace.k, global_scratch); self.v_proj.forward_into(&cn, &mut workspace.v, global_scratch);
        ...
        let kp = pack_f16_to_bits(&workspace.k); c.k[s_o * (n_kv * h_d / 32) .. s_o * (n_kv * h_d / 32) + kp.len()].copy_from_slice(&kp);
        c.v[s_o * (n_kv * h_d) .. s_o * (n_kv * h_d) + workspace.v.len()].copy_from_slice(&workspace.v); c.current_len = nl;
        ...
        let os = unsafe { std::slice::from_raw_parts_mut(out_ptr, x.len()) }; self.o_proj.forward_into(&ao, os, global_scratch);
        for i in 0..x.len() { os[i] += x[i]; }
    ...
    pub fn get_kv_data(&self, h_d: usize, n_kv: usize, s_t: usize) -> Option<(Vec<u32>, Vec<f16>)> {
        ...
        if c.current_len > s_t { let ku = n_kv * (h_d/32); let vu = n_kv * h_d; Some((c.k[s_t * ku .. c.current_len * ku].to_vec(), c.v[s_t * vu .. c.current_len * vu].to_vec())) }
        else { None }
    }
    ...
    pub fn inject_gpu_kv_direct(&self, ks: GpuPtr, vs: GpuPtr, tok: usize, n_kv: usize, h_d: usize) {
        ...
        unsafe {
            ...
            let r1 = cl.cuMemAlloc_v2(&mut kp, kb); let r2 = cl.cuMemAlloc_v2(&mut vp, vb);
            if r1 as i32 != 0 || r2 as i32 != 0 { return; }
...
impl NativeQwen3TextModel {
    ...
    pub fn forward_ext(&self, _i_ids: &[u32], embeds: Vec<f16>, s_o: usize, gs: Option<(&std::sync::Mutex<Option<(GpuPtr, usize)>>, &std::sync::Mutex<Option<(GpuPtr, usize)>>, &std::sync::Mutex<Option<(GpuPtr, usize)>>)>, workspace: Option<&mut ForwardWorkspace>, _sl: Option<&NativeLayer>, is_vision: bool) -> Vec<f16> {
        ...
        for (i, layer) in self.layers.iter().enumerate() {
            let active = layer; let use_b = i % 2 == 0;
        ...
        let n_cow = self.norm.get_slice::<f16>(); let mut cn = vec![f16::ZERO; cur_x.len()];
        native_rms_norm_f16_into(cur_x, n_cow.as_ref(), self.config.rms_norm_eps as f32, hid, &mut cn);
        log_tensor_health("Final Norm Output", &cn, &[q_len, hid]); cn
    }
    pub fn move_to_gpu(&mut self, device_id: i32) -> anyhow::Result<()> { self.embed_tokens.move_to_gpu(device_id)?; self.norm.move_to_gpu(device_id)?; for layer in &mut self.layers { layer.move_to_gpu(device_id)?; } Ok(()) }
    ...
    pub fn batch_upload_stitched_cache(&self, k: Vec<u32>, v: Vec<f16>) {
        ...
        #[cfg(feature = "cuda")]
        if self.layers[0].device_id >= 0 && self.layers[0].device_id < 8 {
    ...
    pub fn get_all_kv(&self, start_idx: usize) -> Vec<(Vec<u32>, Vec<f16>)> { let hd = self.config.head_dim; let nkv = self.config.num_key_value_heads; self.layers.iter().filter_map(|l| l.get_kv_data(hd, nkv, start_idx)).collect() }
}
...
impl NativeVisionModel {
    ...
    pub fn forward<'a>(&self, pv: &[f16], gt: &[u32; 3], rc: &[f16], rs: &[f16], gs: Option<(&std::sync::Mutex<Option<(GpuPtr, usize)>>, &std::sync::Mutex<Option<(GpuPtr, usize)>>, &std::sync::Mutex<Option<(GpuPtr, usize)>>)>, ws: &'a mut ForwardWorkspace, rope_cache_gpu: &std::sync::Mutex<Option<(NativeTensor, NativeTensor)>>) -> &'a [f16] {
        ...
        self.merger.forward(cur_x, &crate::models::qwen3vl::config::Qwen3VLTextConfig {
            hidden_size: cur_x.len() / patches, intermediate_size: cur_x.len() / patches, num_hidden_layers: 1, num_attention_heads: 1, num_key_value_heads: 1, head_dim: cur_x.len() / patches, rms_norm_eps: 1e-6, rope_theta: 10000.0, vocab_size: 0, max_position_embeddings: 32768, dtype: None, rope_scaling: None,
...
impl NativeQwen3VLModel {
    pub fn load(config: Qwen3VLConfig, m_mmap: Arc<Mmap>, v_mmap: Option<Arc<Mmap>>, baking: bool, s_mmap: Option<Arc<Mmap>>, device_id: i32) -> Result<Self> {
        ...
        let get_l = |base: &str, in_f: usize, out_f: usize, l_idx: i32| -> Result<NativeLinear> {
            ...
            let pn = format!("{}.packed", key); let sn = format!("{}.scales", key); let fn_ = format!("{}.format", key);
            if cst.tensor(&pn).is_ok() {
                let vp = cst.tensor(&pn)?; let vs = cst.tensor(&sn)?; let format = cst.tensor(&fn_).map(|t| t.data()[0] as i8).unwrap_or(1);
                let (so, si) = if let Ok(st_shape) = cst.tensor(&format!("{}.shape", key)) { let s_data = unsafe { std::slice::from_raw_parts(st_shape.data().as_ptr() as *const i32, 2) }; (s_data[0] as usize, s_data[1] as usize) }
                                else { let s_out = (vs.data().len() / 2) / ((vp.data().len() / 4 * 32) / (vs.data().len() / 2)); (s_out, (vp.data().len() / 4 * 32) / s_out) };
                let ratio_i = if in_f > si { in_f / si } else { 1 }; let ratio_o = if out_f > so { out_f / so } else { 1 };
                if ratio_i > 1 || ratio_o > 1 {
                    let slice_count = if format == 4 { 4 } else { 1 };
                    let mut np = vec![0u32; (out_f/8)*(in_f/32)*8 * slice_count]; let mut ns = vec![f16::ZERO; (out_f/8)*(in_f/32)*8];
                    ...
                    let skb = si / 32; let dkb = in_f / 32; let snb = so / 8; let dnb = out_f / 8;
                    let sss = (so + 7) / 8 * skb * 8; let dss = (out_f + 7) / 8 * dkb * 8;
                    for sl in 0..slice_count { for no in 0..dnb { let sno = no % snb; for ko in 0..dkb { let sko = ko % skb; for sn_idx in 0..8 {
                        let si_idx = sl * sss + (sno * skb + sko) * 8 + sn_idx; let di_idx = sl * dss + (no * dkb + ko) * 8 + sn_idx;
                        if si_idx < pu.len() { np[di_idx] = pu[si_idx]; if sl == 0 { ns[(no * dkb + ko) * 8 + sn_idx] = sf[(sno * skb + sko) * 8 + sn_idx]; } }
                    } } } }
                    let pb_len = np.len(); let sb_len = ns.len();
                    let pb = np.into_boxed_slice(); let sb = ns.into_boxed_slice(); let pp = pb.as_ptr(); let sp = sb.as_ptr(); std::mem::forget(pb); std::mem::forget(sb);
                    let variant = if format == 4 { LinearVariant::BitSliced4 { weight_packed: NativeTensor { data_ptr: pp as *const u8, host_size: pb_len*4, gpu_ptr: None, shape: vec![out_f * 4, in_f/32], dtype: NativeDType::U32, _mmap: None, device_id: active_gpu_id }, scales: NativeTensor { data_ptr: sp as *const u8, host_size: sb_len*2, gpu_ptr: None, shape: vec![out_f, in_f/32], dtype: NativeDType::F16, _mmap: None, device_id: active_gpu_id }, bias: None } }
                                  else { LinearVariant::BitSerial { weight_packed: NativeTensor { data_ptr: pp as *const u8, host_size: pb_len*4, gpu_ptr: None, shape: vec![out_f, in_f/32], dtype: NativeDType::U32, _mmap: None, device_id: active_gpu_id }, scales: NativeTensor { data_ptr: sp as *const u8, host_size: sb_len*2, gpu_ptr: None, shape: vec![out_f, in_f/32], dtype: NativeDType::F16, _mmap: None, device_id: active_gpu_id }, bias: None } };
                    Ok(NativeLinear { in_features: in_f, out_features: out_f, src_in: si, src_out: so, variant, device_id: active_gpu_id })
                } else {
                ...
                if in_f > si || out_f > so {
                    let sf = unsafe { std::slice::from_raw_parts(v.data().as_ptr() as *const f16, v.data().len()/2) };
                    let mut nd = vec![f16::ZERO; out_f * in_f]; let rk = in_f/si; let es = 1.0/(rk as f32);
                    for r in 0..out_f { for c in 0..in_f { nd[r*in_f+c] = f16::from_f32(sf[(r%so)*si+(c%si)].to_f32() * es); } }
                    let boxed = nd.into_boxed_slice(); let ptr = boxed.as_ptr(); std::mem::forget(boxed);
                    Ok(NativeLinear { in_features: in_f, out_features: out_f, src_in: si, src_out: so, variant: LinearVariant::Standard { weight: NativeTensor { data_ptr: ptr as *const u8, host_size: out_f*in_f*2, gpu_ptr: None, shape: vec![out_f, in_f], dtype: NativeDType::F16, _mmap: None, device_id: active_gpu_id }, bias: None }, device_id: active_gpu_id })
                } else {
        ...
        let get_t = |name: &str, ts: usize| -> Result<NativeTensor> {
            ...
            if ts > sl {
                ...
                let boxed = nd.into_boxed_slice(); let ptr = boxed.as_ptr(); std::mem::forget(boxed);
                Ok(NativeTensor { data_ptr: ptr as *const u8, host_size: ts*2, gpu_ptr: None, shape: vec![ts], dtype: NativeDType::F16, _mmap: None, device_id: active_gpu_id })
            } else {
                let off = unsafe { v.data().as_ptr().offset_from(cm.as_ptr()) } as usize;
                Ok(NativeTensor { data_ptr: unsafe { cm.as_ptr().add(off) }, host_size: v.shape().iter().product::<usize>()*2, gpu_ptr: None, shape: v.shape().to_vec(), dtype: NativeDType::F16, _mmap: Some(cm.clone()), device_id: active_gpu_id })
            }
        };
        let get_embed = |base: &str, vocab: usize, thid: usize| -> Result<NativeLinear> {
            ...
            if cst.tensor(&pn).is_ok() {
                let vp = cst.tensor(&pn)?; let vs = cst.tensor(&sn)?; let format = cst.tensor(&fn_).map(|t| t.data()[0] as i8).unwrap_or(1);
                let sc = if format == 4 { 4 } else { 1 }; let shid = (vp.data().len() / 4 * 32) / (vocab * sc);
                ...
                let dq = dequantize_bit_serial_to_f16(pr, sr, vocab, shid, thid, sc);
                ...
                Ok(NativeLinear { in_features: vocab, out_features: thid, src_in: shid, src_out: thid, variant: LinearVariant::Standard { weight: NativeTensor { data_ptr: ptr as *const u8, host_size: vocab*thid*2, gpu_ptr: None, shape: vec![vocab, thid], dtype: NativeDType::F16, _mmap: None, device_id: active_gpu_id }, bias: None }, device_id: active_gpu_id })
        ...
        if let Some(ref sec_mmap) = s_mmap {
            ...
            support_layer0 = Some(NativeLayer {
                ...
                q_proj: get_l(&format!("{}.self_attn.q_proj.weight", p), t_c.hidden_size, q_out, 0)?,
                k_proj: get_l(&format!("{}.self_attn.k_proj.weight", p), t_c.hidden_size, kv_out, 0)?,
                v_proj: get_l(&format!("{}.self_attn.v_proj.weight", p), t_c.hidden_size, kv_out, 0)?,
                o_proj: get_l(&format!("{}.self_attn.o_proj.weight", p), q_out, t_c.hidden_size, 0)?,
                gate_proj: get_l(&format!("{}.mlp.gate_proj.weight", p), t_c.hidden_size, t_c.intermediate_size, 0)?,
                up_proj: get_l(&format!("{}.mlp.up_proj.weight", p), t_c.hidden_size, t_c.intermediate_size, 0)?,
                down_proj: get_l(&format!("{}.mlp.down_proj.weight", p), t_c.intermediate_size, t_c.hidden_size, 0)?,
                device_id: active_gpu_id, is_support_layer: true, kv_cache: std::sync::Mutex::new(DynamicKVCache::new()), gpu_kv_cache: std::sync::Mutex::new(DynamicGpuKVCache::new()), gpu_broken: std::sync::atomic::AtomicBool::new(false),
            });
        }
        for i in 0..l_to_l {
            ...
            layers.push(NativeLayer {
                ...
                q_proj: get_l(&format!("{}.self_attn.q_proj.weight", p), t_c.hidden_size, q_out, i as i32)?, k_proj: get_l(&format!("{}.self_attn.k_proj.weight", p), t_c.hidden_size, kv_out, i as i32)?, v_proj: get_l(&format!("{}.self_attn.v_proj.weight", p), t_c.hidden_size, kv_out, i as i32)?, o_proj: get_l(&format!("{}.self_attn.o_proj.weight", p), q_out, t_c.hidden_size, i as i32)?,
                gate_proj: get_l(&format!("{}.mlp.gate_proj.weight", p), t_c.hidden_size, t_c.intermediate_size, i as i32)?, up_proj: get_l(&format!("{}.mlp.up_proj.weight", p), t_c.hidden_size, t_c.intermediate_size, i as i32)?, down_proj: get_l(&format!("{}.mlp.down_proj.weight", p), t_c.intermediate_size, t_c.hidden_size, i as i32)?,
        ...
        let visual = if let Some(vm) = v_mmap {
            ...
            let get_vvl = |base: &str, ti: usize, to: usize| -> Result<NativeLinear> {
                let v = vst.tensor(base)?; let o = unsafe { v.data().as_ptr().offset_from(vm.as_ptr()) } as usize;
            ...
            let mut vb = Vec::new(); for i in 0..(if baking { 1 } else { v_cfg.depth }) {
                let p = format!("visual.blocks.{}", i);
                vb.push(NativeLayer {
                    input_layernorm: get_t(&format!("{}.norm1.weight", p), vh)?, post_attention_layernorm: get_t(&format!("{}.norm2.weight", p), vh)?, q_norm: None, k_norm: None,
            ...
            let merger = NativeLayer {
                input_layernorm: get_t("visual.merger.norm.weight", vi)?, post_attention_layernorm: get_t("visual.merger.norm.weight", vi)?, q_norm: None, k_norm: None,
        ...
        #[cfg(feature = "cuda")]
        if active_gpu_id >= 0 {
            let max_dim = t_c.hidden_size.max(t_c.intermediate_size);
            let pb = 2048 * max_dim * 4; unsafe {
                let cl = crate::models::qwen3vl::native_backend::lib();
                let mut p1 = 0; let mut p2 = 0; let mut p3 = 0;
    ...
    pub fn load_kv_stitched(&self, paths: &[std::path::PathBuf]) -> Result<()> {
        ...
        for p in paths {
            if p.exists() && p.is_file() { expanded_paths.push(p.clone()); }
            else {
                ...
                if let Ok(entries) = std::fs::read_dir(dir) {
                    ...
                    let re_num = Regex::new(r"_shard(\d+)").unwrap();
                    shards.sort_by(|a, b| {
                        let get_n = |p: &std::path::Path| { let name = p.file_name().and_then(|s| s.to_str()).unwrap_or(""); re_num.captures(name).and_then(|c| c[1].parse::<usize>().ok()).unwrap_or(0) };
                        get_n(a).cmp(&get_n(b))
                    });
                    expanded_paths.extend(shards);
                }
            }
        }
        ...
        for l_idx in 0..num_target_layers {
            ...
            #[cfg(feature = "cuda")]
            if self.lm_head.device_id >= 0 { gpu_kv.grow(total_tokens, n_kv, h_d, self.lm_head.device_id); }
            let mut current_offset = 0;
            for (p_idx, mmap) in part_mmaps.iter().enumerate() {
                let st = SafeTensors::deserialize(mmap)?; let tokens_in_part = part_metadatas[p_idx];
                let mut file_layers = 0; while st.tensor(&format!("layer.{}.k", file_layers)).is_ok() { file_layers += 1; }
                let src_idx = if file_layers == 1 { 0 } else { l_idx };
                if src_idx >= file_layers { current_offset += tokens_in_part; continue; }
                if let (Ok(kt), Ok(vt)) = (st.tensor(&format!("layer.{}.k", src_idx)), st.tensor(&format!("layer.{}.v", src_idx))) {
                    #[cfg(feature = "cuda")]
                    if self.lm_head.device_id >= 0 {
                        unsafe {
                            let cl = crate::models::qwen3vl::native_backend::lib();
                            if let Some(kp) = gpu_kv.k_ptr { let dk = (kp.0 as u64 + (current_offset * n_kv * (h_d/32) * 4) as u64) as CUdeviceptr; cl.cuMemcpyHtoD_v2(dk, kt.data().as_ptr() as *const _, kt.data().len()); }
                            if let Some(vp) = gpu_kv.v_ptr { let dv = (vp.0 as u64 + (current_offset * n_kv * h_d * 2) as u64) as CUdeviceptr; cl.cuMemcpyHtoD_v2(dv, vt.data().as_ptr() as *const _, vt.data().len()); }
                        }
                    } else {
                        let mut kv_host = layer.kv_cache.lock().unwrap(); kv_host.grow(total_tokens, n_kv, h_d);
                        ...
                        kv_host.k[current_offset * ku .. (current_offset + tokens_in_part) * ku].copy_from_slice(&k_data);
                        kv_host.v[current_offset * vu .. (current_offset + tokens_in_part) * vu].copy_from_slice(&v_data);
                    }
                }
                current_offset += tokens_in_part;
            }
            gpu_kv.current_len = total_tokens; if self.lm_head.device_id < 0 { layer.kv_cache.lock().unwrap().current_len = total_tokens; }
        }
        Ok(())
    }
    ...
    pub fn forward(&self, i_ids: &[u32], pv: Option<&[f16]>, gt: Option<&[u32; 3]>, so: usize) -> Vec<f16> {
        ...
        let embeds = match &self.text_model.embed_tokens.variant {
            ...
            _ => vec![f16::ZERO; i_ids.len() * self.text_model.config.hidden_size]
        };
        let sc = Some((&self.global_scratch_i, &self.global_scratch_o, &self.global_scratch_r));
        let mut final_embeds = embeds;
        if let (Some(pv_val), Some(gt_val)) = (pv, gt) {
            if let Some(ref v) = self.visual {
                ...
                let vf = v.forward(pv_val, gt_val, &rc, &rs, sc, &mut wl, &self.text_model.rope_cache_gpu);
                let hid = self.text_model.config.hidden_size; let mut vidx = 0;
                for (i, &id) in i_ids.iter().enumerate() { if id == 151655 && vidx < (vf.len()/hid) { final_embeds[i*hid..(i+1) * hid].copy_from_slice(&vf[vidx*hid..(vidx+1)*hid]); vidx += 1; } }
            }
        }
        let nx = self.text_model.forward_ext(i_ids, final_embeds, so, sc, Some(&mut wl), None, is_vision);
        if is_baking { return vec![]; }
    ...
    pub fn get_all_kv(&self, start_idx: usize) -> Vec<(Vec<u32>, Vec<f16>)> { let hd = self.text_model.config.head_dim; let nkv = self.text_model.config.num_key_value_heads; if self.text_model.layers.len() <= 1 && self.support_layer0.is_some() { if let Some(kv) = self.support_layer0.as_ref().unwrap().get_kv_data(hd, nkv, start_idx) { return vec![kv]; } } self.text_model.get_all_kv(start_idx) }
}
