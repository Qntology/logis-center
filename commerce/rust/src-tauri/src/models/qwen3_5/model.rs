...
impl Qwen3_5Attention {
    ...
    pub fn forward(&mut self, xs: &Tensor, cos: &Tensor, sin: &Tensor, attention_mask: Option<&Tensor>, seqlen_offset: usize) -> Result<Tensor> {
        ...
        for block in &self.kv_blocks {
            ...
            // 🌟 [CRITICAL FIX] 1024 크기로 뭉텅이 할당된 메모리에서 '진짜 데이터(b_len)'만 정밀하게 칼질(narrow)하여 꺼냅니다.
            // 이 처리가 없으면 AI가 뒷부분의 쓰레기(Zero/Garbage) 메모리를 참조하여 외계어를 내뱉는 "기억상실증"이 발생합니다!
            let k = if k_block.dim(2)? > b_len { k_block.narrow(2, 0, b_len)?.contiguous()? } else { k_block };
            let v = if v_block.dim(2)? > b_len { v_block.narrow(2, 0, b_len)?.contiguous()? } else { v_block };
                ...
                for block in &mut attn.kv_blocks {
                    ...
                    if should_evacuate && inner.k_cache.is_some() && inner.location == crate::models::qwen::quantized_model::KVLocation::VRAM {
                        let (k_cpu, v_cpu) = {
                            ...
                            // 🌟 [CRITICAL FIX] 디스크에 구울 때 쓰레기값이 딸려가서 다음 로딩 때 뇌를 오염시키지 않도록 진짜 길이만큼만 잘라냅니다.
                            let k = if k_raw.dim(2).unwrap_or(0) > b_len { k_raw.narrow(2, 0, b_len).unwrap_or(k_raw.clone()) } else { k_raw.clone() };
                            let v = if v_raw.dim(2).unwrap_or(0) > b_len { v_raw.narrow(2, 0, b_len).unwrap_or(v_raw.clone()) } else { v_raw.clone() };
