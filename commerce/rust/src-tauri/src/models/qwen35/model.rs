...
impl Qwen3_5GatedDeltaNet {
    ...
    fn torch_chunk_gated_delta_rule(
        ...
        let (query, key, value, beta, g): (Tensor, Tensor, Tensor, Tensor, Tensor) =
...
impl Qwen3_5Model {
                    ...
                    Ok(indeices) => {
                        ...
                        for (j, &token) in vision_tokens.iter().enumerate() {
                            if token == image_token_id {
                                if let Some(img_thw) = image_grid_thw {
                                    thw = img_thw.i(image_index)?.to_vec1::<u32>()?;
                                    image_index += 1;
                                    text_end = vision_indices_vec[j];
                                }
                            }
                            if token == video_token_id {
                                if let Some(vid_thw) = video_grid_thw.as_ref() {
                                    thw = vid_thw.i(video_index)?.to_vec1::<u32>()?;
                                    text_end = vision_indices_vec[j];
                                    video_index += 1;
                                }
                            }
    ...
    pub fn forward(
        ...
        if let (Some(visual), Some(pixel_values_video), Some(video_grid_thw)) = (&self.visual, pixel_values_video, video_grid_thw) {
