...
use candle_core::{D, IndexOp, Tensor, DType, Device};
use candle_nn::{
...
pub fn dequantize_q2(packed: &Tensor, scales: &Tensor, shape: &[usize]) -> Result<Tensor> {
    let dev = packed.device();
    let packed_data = packed.to_vec1::<u8>()?;
    let scales_data = scales.to_dtype(DType::F32)?.to_vec1::<f32>()?;
    
    let num_elements = shape.iter().product::<usize>();
    let mut out_data = vec![0.0f32; num_elements];
    
    let block_size = 32;
    for (i, &p) in packed_data.iter().enumerate() {
        let vals = [
            (p >> 6) & 0x3,
            (p >> 4) & 0x3,
            (p >> 2) & 0x3,
        ...
        for (j, &v) in vals.iter().enumerate() {
            ...
            if idx < num_elements {
                ...
                let scale = scales_data[block_idx];
                out_data[idx] = (v as f32 - 1.5) * scale;
            }
        }
    }
    
    Tensor::from_vec(out_data, shape, dev)
}

pub fn load_split_vb(path: &Path, dtype: DType, device: &Device) -> Result<VarBuilder> {
    let tensors = candle_core::safetensors::load(path, &Device::Cpu)?;
    let mut dequantized = std::collections::HashMap::new();
    
    for (name, tensor) in tensors.iter() {
        if name.ends_with(".q2_packed") {
            let base_name = name.strip_suffix(".q2_packed").unwrap();
            let scales = tensors.get(&format!("{}.q2_scales", base_name)).ok_or(anyhow!("Missing scales"))?;
            let shape_t = tensors.get(&format!("{}.q2_shape", base_name)).ok_or(anyhow!("Missing shape"))?;
            let shape_vec = shape_t.to_vec1::<i32>()?.iter().map(|&x| x as usize).collect::<Vec<_>>();
            
            let dq = dequantize_q2(tensor, scales, &shape_vec)?;
            dequantized.insert(base_name.to_string(), dq.to_device(device)?.to_dtype(dtype)?);
        } else if !name.contains(".q2_scales") && !name.contains(".q2_shape") {
            dequantized.insert(name.clone(), tensor.to_device(device)?.to_dtype(dtype)?);
        }
    }
    
    Ok(VarBuilder::from_tensors(dequantized, dtype, device))
}
...
pub struct Qwen3_5TextModel {
    ...
    layers: Vec<Option<Qwen3_5DecoderLayer>>,
    norm: Qwen3_5RMSNorm,
...
impl Qwen3_5TextModel {
    pub fn new(vb: VarBuilder, config: &Qwen3_5TextConfig, base_path: String) -> Result<Self> {
    ...
    pub fn load_layer(&mut self, layer_idx: usize) -> Result<()> {
        if self.layers[layer_idx].is_some() {
            return Ok(());
        }

        let layer_path = Path::new(&self.base_path).join(format!("layer_{}.st", layer_idx));
        let vb = load_split_vb(&layer_path, self.dtype, &self.device)?;
        // Qwen3.5-Split-Q2 uses full names like "model.language_model.layers.0.input_layernorm.weight"
        let vb_layer = vb.pp("model.language_model.layers").pp(layer_idx);
        
        let layer = Qwen3_5DecoderLayer::new(vb_layer, &self.config, layer_idx)?;
        self.layers[layer_idx] = Some(layer);
        Ok(())
    }
    ...
    pub fn forward(&mut self, inputs_embeds: &Tensor, position_ids: &Tensor) -> Result<Tensor> {
        ...
        for i in 0..self.config.num_hidden_layers {
            self.load_layer(i)?;
            let layer = self.layers[i].as_mut().unwrap();
            ...
            // [LAYER-BY-LAYER] Immediately drop weights after computation
            self.layers[i] = None;
        }
    ...
    pub fn clear_cache(&mut self) {
        for layer_opt in self.layers.iter_mut() {
...
impl Qwen3_5Model {
    pub fn new(vb: VarBuilder, config: Qwen3_5Config, base_path: String, is_vision_enabled: bool) -> Result<Self> {
        let vb_m = vb.pp("model");
        
        let visual = if is_vision_enabled {
            let vision_path = Path::new(&base_path).join("vision.st");
            let vb_v = if vision_path.exists() {
                load_split_vb(&vision_path, vb.dtype(), vb.device())?.pp("model.visual")
            } else {
                vb_m.pp("visual")
            };
        ...
        let language_model = Qwen3_5TextModel::new(vb_m.pp("language_model"), &config.text_config, base_path)?;
