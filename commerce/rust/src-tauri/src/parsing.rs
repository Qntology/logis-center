...
pub fn image2json(region: &str, language: &str, page_type: &str, address: &str) -> String {
    if page_type == "tracking" {
		...
		#tracking_number(tracking number | 운송장 번호 | 송장 번호 | 송장번호 | 등기 번호 | 등기번호 | 运单号 | 運單號 | 伝票番号 | Número de seguimiento | Numéro de suivi | Sendungsnummer | Номер накладной | Número de rastreamento | Numero di tracciamento | رقم التتبع  | Số vận đơn | Nomor resi | หมายเลขติดตามพัสดุ) is selected from the number that matches the barcode | QR code, among others, based on the #region, excluding the format of a national telephone | mobile phone number, | an order number
        #recipient_match : shipping label #recipient_address match. Ruled the same despite different floor levels
		{
			tracking_number:'',
			recipient_match : boolean,
			barcodes : [barcode number | string],
			text : summarize the shipping label contents in {LANGUAGE}. Masking the address in the summary to District-level and up. Do not mention that information is masked or partially hidden,
		}"###;
...
pub fn item2json(page_type: &str, href: &str, language: &str) -> Vec<(String, String)> {
    ...
    match page_type {
        "tracking" => {
            fields.push(("status".to_string(), r#"\n            status:{\n                value:'draft' | 'progress' | 'return' | 'complete' | 'error',\n                selector:selector\n            }"#.to_string()));
            ...
            fields.push(("shipping_method".to_string(), r#"\n            shipping_method:{\n                value:'standard' | 'express' | 'same_day' | 'pick_up' | 'freight' | 'prepaid',\n                selector:selector\n            }"#.to_string()));
        ...
        "goods" => {
            ...
            fields.push(("status".to_string(), r#"\n            status:{\n                value:'draft' | 'show' | 'hide' | 'progress' | 'stop' | 'cancel' | 'refund' | 'return' | 'exchange' | 'expire' | 'complete' | 'error',\n                selector:selector\n            }"#.to_string()));
            ...
            fields.push(("condition".to_string(), r#"\n            condition:{\n                value:['new' | 'used' | 'lease' | 'rental' | 'refurbish'],\n                selector:selector\n            }"#.to_string()));
        ...
        "order" => {
            ...
            fields.push(("payment_method".to_string(), r#"\n            payment_method:{\n                value:'C.O.D.' | 'CARD' | 'BANK' | '' | string,\n                selector:selector\n            }"#.to_string()));
            fields.push(("payment_origin".to_string(), r#"\n            payment_origin:{\n                value:Payment Gateway Service Name | '' | string,\n                selector:selector\n            }"#.to_string()));
            fields.push(("registration_date".to_string(), r#"\n            registration_date:{\n                value:yyyy-MM-ddThh:mm:ss | string,\n                selector:selector\n            }"#.to_string()));
        },
        "coupon" | "event" => {
            ...
            fields.push(("status".to_string(), r#"\n            status:{\n                value:'draft' | 'progress' | 'stop' | 'cancel' | 'expire' | 'complete' | 'error',\n                selector:selector\n            }"#.to_string()));
        ...
        "review" => {
            ...
            fields.push(("status".to_string(), r#"\n            status:{\n                value:'progress' | 'stop' | 'cancel' | 'refund' | 'return' | 'exchange' | 'expire' | 'complete' | 'error',\n                selector:selector\n            }"#.to_string()));
