// =====================================================================
// 🌟 [LEGIBILITY MAP] 패치 격자 단위 '판독 가능성' 지도
// ---------------------------------------------------------------------
//  ── 왜 필요한가 ──
//   실측 인보이스의 EXPORTER / CONSIGNEE 블록(y 255~360)은 블러 처리되어
//   사람도 읽을 수 없습니다. 그런데 파이프라인은 그 영역을 정상 텍스트와
//   구분하지 못하므로, 크롭이 거기에 착지하면 2B 모델이 '무언가를 지어냅니다'.
//   같은 문제가 빈 셀에서도 일어납니다. BUYER (IF NOT CONSIGNEE) 박스는
//   내용이 비어 있는데, 모델이 헤더 라벨을 값으로 읽어
//   recipient_name = "BUYER (IF NOT CONSIGNEE)" 를 반환했습니다.
//
//  ── 왜 임베딩이 아니라 픽셀인가 ──
//   블러는 '의미' 가 아니라 '고주파 성분의 소실' 입니다.
//   SigLIP2 패치 임베딩도 흐려지긴 하지만, 그 흐려짐이 '개념 부재' 인지
//   '해상도 부족' 인지 구분할 수 없습니다.
//   반면 휘도 기울기 에너지는 블러를 물리적으로 직접 측정합니다.
//
//  ── 매직 상수가 없는 이유 ──
//   판정 임계는 Otsu 이분법으로 '이 문서 자신의 분포' 에서 유도합니다.
//   Otsu 는 클래스 간 분산을 최대화하는 지점을 찾는 무모수 방법이며,
//   분포가 단봉이면(= 블러 영역이 없으면) 분리를 기각하도록 설계했습니다.
// =====================================================================
